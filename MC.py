...
if __name__ == '__main__':
    ...
    xT_init = config.Sampler.tmax * torch.randn(args.sampN, config.model.ndim).to(args.device)
    if args.M == 1:
        eps_init = torch.randn(args.sampN, config.model.ndim).to(args.device)
    else:
        eps_init = torch.randn(args.M, args.sampN, config.model.ndim).to(args.device)
    if if_com:
    ...
    for i in range(0, config.MC.mc_steps):
        ...
        if i % config.MC.save_steps == 0:
            ...
            logger.info(f'Step {i}/{config.MC.mc_steps}')
    ...
    logger.info(f'x0 rate:, {(x0[:, 0] < 0.0).sum().item()/args.sampN}')
