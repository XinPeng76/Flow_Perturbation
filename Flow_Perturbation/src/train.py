...
def train_model_DDPM_R(model, n_particles, n_dimensions, dataloader, path, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, num_steps,num_epoch=10001, lr=1e-4, loss_DDPM = diffusion_loss_fn_v_prediction,decay_steps = 1000):
    if not os.path.exists(path):
        os.makedirs(path)
    seed = 1234
    print('Training model...')
    #model.load_state_dict(torch.load('models/CGN1/model_RotAug_LowLR.pth'))		
    optimizer = torch.optim.Adam(model.parameters(),lr=lr)
...
def train_model_EDM_R(model, n_particles, n_dimensions, device, ndim, sig_data, dataloader, path,num_epoch=81,lr=1e-4, loss_EDM = loss_EDM,decay_steps = 20):
    ...
    for t in range(num_epoch):
        ...
        for idx,batch_x in enumerate(dataloader):
            batch_x = rotate_molecules(batch_x, n_particles, n_dimensions)
            loss = loss_EDM(model, n_particles, n_dimensions, sig_data, device, batch_x)
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.)	
            optimizer.step()
