...
def exact_dynamics_heun_dSt_Huch(xtn1): # do a lot of Heun steps between tn and tn1 to get the accurate xtn
    ...
    _, div_xt = get_vjp_score(xt, ts[len(ts)-1], eps)
    #print(score_xt)
    for i in range(len(ts)-1, 0, -1):
        xt_new = heun_torch(xt,ts[i], ts[i-1])

        eps = torch.randn_like(xt_new)
        _, div_xt_new = get_vjp_score(xt_new, ts[i-1], eps)
...
if __name__ == '__main__':
    ...
    torch.save(state_dict, f'data/CGN-Hutch1-{back_coeff}-{K_x}.pth')

    concatenated_x0_last_steps = torch.cat(x0_last_steps, dim=0)
    torch.save(concatenated_x0_last_steps, f'data/x0_last_steps_CGN-Hutch1-{back_coeff}-{K_x}.pth')
    torch.save(energy, f'data/energy_CGN-Hutch1-{back_coeff}-{K_x}.pt')
